...
`npm run lint`:: check the code for style and formatting problems
`npm run typecheck`:: check the TypeScript code for type errors, both under our own settings and under the looser ones downstream consumers compile with
...
`npm run dist`:: does everything necessary to update the `dist/` directory with the latest changes, including lint, test, typecheck, build(s), and docs
